...
st.set_page_config(page_title="产研团队人才盘点系统", page_icon="📊", layout="wide")
...
def get_education_tier(row):
    school_type = row['学历-学校类别']
    qs_rank = row['海外高校QS排名']
...
def get_performance_tier(row):
    ...
    for col in annual_cols:
        if pd.notna(row[col]) and row[col] in ['S', 'A']:
            has_annual_sa = True
            break
...
def is_high_potential(row):
    ...
    young = row['司龄'] <= 1
    level_ok = row['职级'] in ['L3', 'L4', 'L5', 'L6', '培训生']
...
def is_stable(row):
    ...
    stable_time = row['司龄'] >= 0.5
    return all_b_plus and stable_time

def is_core_backbone(row):
    class_ok = row['岗位分类'] in ['A类', 'M类']
    perf_ok = row['绩效档位'] in ['一档', '二档']
    time_ok = row['司龄'] >= 0.5
    level_ok = row['职级'] in ['L6', 'L7', 'L8', 'L9', 'M4']
    match_ok = row['匹配度'] == '完全匹配'
...
def is_resignation_risk(row):
    ...
    if row['司龄'] < 0.5:
        ...
        if any(p in ['B', 'C'] for p in performances):
            return True
    if (row['待关注'] or row['待优化']) and row['岗位分类'] == 'A类':
        return True
    return False
...
def generate_excel_report(df, stats):
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        report_data = [
            ...
            ['3', f'人岗匹配度：完全匹配{len(df[df["匹配度"]=="完全匹配"])}人({len(df[df["匹配度"]=="完全匹配"])/total_employees*100:.1f}%)，不匹配{len(df[df["匹配度"]=="不匹配"])}人({len(df[df["匹配度"]=="不匹配"])/total_employees*100:.1f}%)'],
        ]
        ...
        match_stats = df['匹配度'].value_counts()
        match_data = [
            ['人岗匹配度校验明细：'],
            [],
            ['完全匹配', f'{match_stats.get("完全匹配", 0)}人', f'{match_stats.get("完全匹配", 0)/total_employees*100:.1f}%'],
            ['基本匹配', f'{match_stats.get("基本匹配", 0)}人', f'{match_stats.get("基本匹配", 0)/total_employees*100:.1f}%'],
            ['不匹配', f'{match_stats.get("不匹配", 0)}人', f'{match_stats.get("不匹配", 0)/total_employees*100:.1f}%'],
            [],
            ['不符合学历+绩效标准人员清单：'],
        ]
        ...
        df[['工号', '姓名', '部门', '岗位', '职位', '职级', '岗位分类', '学历档位', '绩效档位', '匹配度', '司龄', 
            '高潜人才', '稳定人员', '核心骨干', '待关注', '待优化', '离职风险']].to_excel(
            writer, sheet_name='十六、完整盘点数据', index=False)
...
def generate_pdf_report(df, stats):
    try:
        ...
        chinese_font = None
        if os.path.exists(font_file) and os.path.getsize(font_file) > 1000000:
            try:
        ...
        if chinese_font:
            ...
            highlight_style = ParagraphStyle('HighlightStyle', parent=styles['Normal'], fontName=chinese_font, fontSize=11, textColor=colors.HexColor('#E74C3C'), leading=15)
            ...
            highlight_style = ParagraphStyle('HighlightStyle', parent=styles['Normal'], fontSize=11, textColor=colors.HexColor('#E74C3C'), leading=15)
            font_name = 'Helvetica'
        ...
        if chinese_font:
            ...
            match_data = [
                ...
                ['Full Match', f'{stats["full_match"]}', f'{stats["full_match_rate"]:.1f}%', 'Excellent'],
                ['Basic Match', f'{stats["basic_match"]}', f'{stats["basic_match_rate"]:.1f}%', 'Meets requirements'],
                ['No Match', f'{stats["no_match"]}', f'{stats["no_match_rate"]:.1f}%', 'Needs attention'],
            ]
        ...
        if chinese_font:
            ...
            talent_data = [
                ...
                ['High Potential', f'{stats["high_potential"]}', f'{stats["high_potential_rate"]:.1f}%', 'New + Excellent performance'],
                ['Stable', f'{stats["stable"]}', f'{stats["stable_rate"]:.1f}%', 'Consistent good performance'],
                ['Core Backbone', f'{stats["core_backbone"]}', f'{stats["core_backbone_rate"]:.1f}%', 'Key senior staff'],
                ['Need Attention', f'{stats["attention"]}', f'{stats["attention_rate"]:.1f}%', 'Performance fluctuation'],
                ['Need Optimization', f'{stats["optimization"]}', f'{stats["optimization_rate"]:.1f}%', 'Performance issues'],
                ['Resignation Risk', f'{stats["risk"]}', f'{stats["risk_rate"]:.1f}%', 'High turnover risk'],
            ]
        ...
        if stats['edu_fail'] > 0 and chinese_font:
        ...
        if stats['optimization'] > 0:
            ...
            for _, row in opt_df.iterrows():
                opt_data.append([row['姓名'], row['部门'][:8], row['岗位'][:10], row['职级'], row['岗位分类'], row['匹配度']])
        ...
        if stats['risk'] > 0:
            ...
            for _, row in risk_df.iterrows():
                risk_data.append([row['姓名'], row['部门'][:8], row['岗位'][:10], row['职级'], row['岗位分类'], f'{row["司龄"]:.1f}年'])
        ...
        if stats['high_potential'] > 0:
            ...
            for _, row in hp_df.iterrows():
                hp_data.append([row['姓名'], row['部门'][:8], row['岗位'][:10], row['职级'], row['岗位分类'], f'{row["司龄"]:.1f}年'])
        ...
        footer_style = ParagraphStyle('FooterStyle', parent=styles['Normal'], fontName=font_name, fontSize=9, textColor=colors.HexColor('#95A5A6'), alignment=1)
        if chinese_font:
...
st.sidebar.title("📊 人才盘点系统")
page = st.sidebar.radio("导航", ["首页", "数据上传", "盘点分析", "报告导出"])
    ...
    if st.session_state.result_df is None:
        ...
        with col2:
            ...
            if st.button("生成PDF报告", type="primary"):
                with st.spinner("正在生成PDF报告（首次可能需要下载字体，请耐心等待）..."):
