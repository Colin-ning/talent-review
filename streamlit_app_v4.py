...
def generate_pdf_report(df, stats):
    try:
        ...
        title1 = '一、盘点概览' if font_available else '1. Overview'
        pdf.cell(0, 10, txt=title1, ln=True)
        pdf.set_font(font_name, '', 10)
        
        items = [
            (f'盘点总人数：{stats["total"]}人', f'Total: {stats["total"]}'),
            (f'部门覆盖：{stats["departments"]}个', f'Departments: {stats["departments"]}'),
            (f'完全匹配：{stats["full_match"]}人 ({stats["full_match_rate"]:.1f}%)', f'Full Match: {stats["full_match"]} ({stats["full_match_rate"]:.1f}%)'),
            (f'待优化：{stats["optimization"]}人', f'Need Optimization: {stats["optimization"]}'),
            (f'离职风险：{stats["risk"]}人', f'Risk: {stats["risk"]}'),
        ]
        for cn, en in items:
            pdf.cell(0, 7, txt=cn if font_available else en, ln=True)
        
        pdf.ln(8)
        pdf.set_font(font_name, '', 14)
        title2 = '二、人才分层统计' if font_available else '2. Talent Categories'
        pdf.cell(0, 10, txt=title2, ln=True)
        pdf.set_font(font_name, '', 10)
        
        talent_items = [
            (f'高潜人才：{stats["high_potential"]}人', f'High Potential: {stats["high_potential"]}'),
            (f'稳定人员：{stats["stable"]}人', f'Stable: {stats["stable"]}'),
            (f'核心骨干：{stats["core_backbone"]}人', f'Core Backbone: {stats["core_backbone"]}'),
            (f'待关注：{stats["attention"]}人', f'Need Attention: {stats["attention"]}'),
            (f'待优化：{stats["optimization"]}人', f'Need Optimization: {stats["optimization"]}'),
            (f'离职风险：{stats["risk"]}人', f'Risk: {stats["risk"]}'),
        ]
        for cn, en in talent_items:
            pdf.cell(0, 7, txt=cn if font_available else en, ln=True)
        
        pdf.ln(8)
        pdf.set_font(font_name, '', 14)
        title3 = '三、核心建议' if font_available else '3. Recommendations'
        pdf.cell(0, 10, txt=title3, ln=True)
        pdf.set_font(font_name, '', 10)
        
        recs = [
            (f'1. 高潜人才培养：{stats["high_potential"]}人', f'1. High potential: {stats["high_potential"]}'),
            (f'2. 核心骨干保留：{stats["core_backbone"]}人', f'2. Core backbone: {stats["core_backbone"]}'),
            (f'3. 待优化处理：{stats["optimization"]}人', f'3. Need optimization: {stats["optimization"]}'),
            (f'4. 离职风险防控：{stats["risk"]}人', f'4. Risk: {stats["risk"]}'),
        ]
        for cn, en in recs:
            pdf.cell(0, 7, txt=cn if font_available else en, ln=True)
