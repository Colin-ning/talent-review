...
def generate_pdf_report(df, stats):
    try:
        ...
        pdf.cell(0, 10, txt='一、盘点概览', ln=True)
        pdf.set_font('Chinese', '', 10)
        
        items = [
            f'盘点总人数：{stats["total"]}人',
            f'部门覆盖：{stats["departments"]}个',
            f'完全匹配：{stats["full_match"]}人 ({stats["full_match_rate"]:.1f}%)',
            f'待优化：{stats["optimization"]}人',
            f'离职风险：{stats["risk"]}人',
        ]
        for item in items:
        ...
        pdf.ln(8)
        pdf.set_font('Chinese', '', 14)
        pdf.cell(0, 10, txt='二、人才分层统计', ln=True)
        pdf.set_font('Chinese', '', 10)
        
        talent_items = [
            f'高潜人才：{stats["high_potential"]}人',
            f'稳定人员：{stats["stable"]}人',
            f'核心骨干：{stats["core_backbone"]}人',
            f'待关注：{stats["attention"]}人',
            f'待优化：{stats["optimization"]}人',
            f'离职风险：{stats["risk"]}人',
        ]
        ...
        pdf.ln(8)
        pdf.set_font('Chinese', '', 14)
        pdf.cell(0, 10, txt='三、核心建议', ln=True)
        pdf.set_font('Chinese', '', 10)
